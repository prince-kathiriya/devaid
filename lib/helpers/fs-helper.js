...
//
function readdirrecur(projectRootDir, _path, ignoreExists, res = {}, _pathToSkeleton = arguments[1], indent = '') {
  ...
  for (let idx = 0; idx < dirs.length; idx++) {
    ...
    const pathToLog = __path.split(_pathToSkeleton)[1].split('/').pop();
    res.log = (res.log || '') +  
      (
        indent + (
    ...
    if (!ignoreExists.includes(__path.split(_pathToSkeleton)[1])) {
      const pathToBeCreated = path.join(projectRootDir, __path.split(_pathToSkeleton)[1]);
    ...
    readdirrecur(projectRootDir, path.resolve(_path, dirs[idx]), ignoreExists, res, _pathToSkeleton, indent + '│   ');
  }
  return res;
}
...
//
async function getDirsToCreate({ pathToSkeleton, root = process.cwd(), ignoreExists = [] } = {}) {
  const { log: dirLogs, exists: dirExists, ...dirs } = readdirrecur(root, pathToSkeleton, ignoreExists);
  ...
  if (!isConfirmed) {
      console.log(`\n\n ${chalk.yellow(csts.PREFIX_MSG_WARNING)}'Project creation cancelled by user!' \n`);
      return null;
  }
  return dirs;
}
