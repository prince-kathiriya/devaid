...
function toAllCaseFormats(input) {
  ...
  return {
    ...
    pascalCase: input.replace(/-([a-z])/g, (g) => g[1].toUpperCase()).replace(/^[a-z]/, (g) => g.toUpperCase()),
...
function mapChoices({ choices } = {}) {
  const disabledIdxStart = (choices?.disabledIdxStart ?? disabledIdxStart) - 1;
...
async function getVariableName({ name, message, isExitFlag = true, hint } = {}) {
  ...
  do {
    if (!isValidInput) hint = `:-- Please enter valid ${message.toLowerCase()}.`;
