...
const upload = {
  ...
  //! dbFieldName is the name of the field in database where file path will be stored
  single({ fieldName, dbFieldName = 'image' } = {}) {
    return async (req, res, next) => {
      ...
      uploadMethods[multerUploadMethod].single(fieldName)(req, res, (error) => {
        ...
        req.body[dbFieldName] = req?.file?.location || req?.file?.path;
        next();
      });
    };
  },
  ...
  array({
    fieldName, maxCount = 10, dbFieldName = 'images', isSRCFormat = true,
  } = {}) {
    return async (req, res, next) => {
      const multerUploadMethod = req.headers['local-upload'] === 'true' ? 'multerLocal' : 'multerS3';
      ...
      uploadMethods[multerUploadMethod].array(fieldName, maxCount)(
        req, res, (error) => {
          if (error) return errorHandler({ req, res, error });
          req.body[dbFieldName] = isSRCFormat
            ? req?.files?.map((item) => ({
  ...
  fields(
    { fields } = {
      fields: [
        { name: 'image', maxCount: 1, dbFieldName: 'image', isArray: true, isSRCFormat: true },
      ],
    }
  ) {
    return async (req, res, next) => {
      ...
      uploadMethods[multerUploadMethod].fields(fields)(req, res, (error) => {
        ...
        for (let i = 0; i < fields.length; i++) {
          ...
          req.body[fields[i].dbFieldName] = fields[i].isArray
            ? isSRCFormat
