...
const setupcrud = {
    ...
    getSchema: async ({ database, models }, root = {}, inType = 'Object', depth = 0, prefix = '') => {
        ...
        let variableName;
        if (inType !== 'Array') {
            variableName = await helper.cli.getVariableName({ name: 'variableName', message: 'Schema field name', hint: `${prefix ? ` | ${prefix}` : `'root'`}->(::enter schema field name) | e.g. fooBar | enter -1 to exit` });
            ...
            if (!isOverwrite && Object.keys(root).includes(variableName)) {
                console.log('\n ', helper.csts.PREFIX_MSG_ERR, chalk.red(`Schema must have unique fields, '${variableName}' field already exists.\n`));
                return await setupcrud.getSchema({ database, models }, root, inType, depth, prefix);
            }
            
            if (+variableName === -1) {
                if (Object.keys(root).length === 0 && depth === 0) {
                    console.log('\n ', helper.csts.PREFIX_MSG_ERR, chalk.red('Schema must have atleast one field.\n'));
                    return await setupcrud.getSchema({ database, models }, root, inType, depth, prefix);
                } else {
                    return root;
                }
        ...
        function inner(rawdata, prefix = '') {
          if (typeof rawdata !== 'object') {
            ...
            return [{
              ...
              ...(
                  !isImage 
                    ? { value: setupcrud.cst[database].sampleData[rawdata.split('__')[0]] }
    ...
    action: async (options) => {
        try {
            ...
            //! add crud name to dbmodels array
            intake.dbmodels.push(intake.crudNameCase.pascalCase); 
            intake.dbmodels = [...new Set(intake.dbmodels)];

            console.log('\n ', helper.csts.PREFIX_MSG_INFO, chalk.cyan(`Let's setup schema for '${intake.crudNameCase.pascalCase}' model.\n`));
