    ...
    action: async (options) => {
        try {
            ...
            if (helper.fs.isDirExists({ path: projectDir })) {
                console.log(`\n ${helper.csts.PREFIX_MSG_WARNING}${chalk.yellow(`Project directory already exists, path: `)}${chalk.white.bold(projectDir)}\n`);
            
                const isConfirmed = await helper.cli.getConfirmation({ message: 'Do you want to overwrite existing project? (Y/N)' });
                if (!isConfirmed) {
                    console.log(`\n ${helper.csts.PREFIX_MSG}${chalk.green('Please try again with different project name or change directory!')}\n`);
            ...
            console.log(`\n${helper.csts.PREFIX_MSG_INFO}${chalk.cyan('Run following commands to start the server:')}`);
            ...
            console.log(`  ${chalk.blue.bold(`4. mv .gitignore.example .gitignore`)}`);
            console.log(`  ${chalk.blue.bold(`5. open .env file and fill out the variables`)}`);
            console.log(`  ${chalk.blue.bold('6. npm run dev')}`);

            console.log(`\n${helper.csts.PREFIX_MSG_INFO}${chalk.cyan('Steps to setup postman collection:')}`);
            ...
            console.log(`  ${chalk.blue.bold(`5. test server by running 'server-check' request in postman`)}`);
